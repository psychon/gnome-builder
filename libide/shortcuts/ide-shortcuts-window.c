                             ...
                             gpointer      callback_data)
{
  ...
  if (include_internal)
    {
      GTK_CONTAINER_CLASS (ide_shortcuts_window_parent_class)->forall (container, include_internal, callback, callback_data);
    }
  else
    {
      if (priv->stack)
...
ide_shortcuts_window_init (IdeShortcutsWindow *self)
{
  ...
  priv->main_box = g_object_new (GTK_TYPE_BOX,
                           ...
                           "visible", TRUE,
                           NULL);
  GTK_CONTAINER_CLASS (ide_shortcuts_window_parent_class)->add (GTK_CONTAINER (self), GTK_WIDGET (priv->main_box));
